...
class MySQLUtilities:
    ...
    def select_all_query(table_name: str):
        """ Get all results query """
        query = f"""SELECT * FROM {table_name} LIMIT 25;"""
        return query
    
    @staticmethod
    def is_database_created(database_name: str, cursor_object) -> bool:
        """
        ...
        databases: list = [database[0] for database in cursor_object.fetchall()] 
    ...
    @staticmethod
    def __display_results(table_column_names: list[str], results: list[list], exec_time: float, result_limit) -> None:
        """
        ...
        is_same = len(results) == result_limit  # Check if the len of results returned is not the same as the result limit
    ...
    def execute_display_query_results(query: str, cursor_object, logger: Optional[object] = None, result_limit: int = 50) -> None:
        ...
        MySQLUtilities.__display_results(table_column_names, results, round(exec_time, 3), result_limit)
